import connectDataBase from "../conect-data/conectDB.js";
...
class Rotas {
  static async methodGet(req, res){ //GET OK
    try {
      ...
      if (id) {
          const data = await connectDataBase.findOne({'_id':ObjectIdDB});
          res.status(200).json(data); 
        }else{
          const data = await connectDataBase.find({}).toArray();
          res.status(200).json(data);       
        }
      }
      catch(error){
    ...
    static async methodPost (req, res) { //POST OK
      try {
        const data = await connectDataBase.insertOne(req.body);
    ...
    static async methodPetch(req, res){
      try {
        ...
        await connectDataBase.findOneAndUpdate({'_id':ObjectIdDB}, { $set:req.body });
    ...
    static async methodDelete(req, res) { //DELETE OK
      try {
        ...
        await connectDataBase.findOneAndDelete({'_id':ObjectIdDB});
